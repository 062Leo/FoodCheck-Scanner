...
export function ContributeForm({
  ...
  const [saturatedFat100g, setSatFat] = useState(
    initialNutriments?.saturatedFat100g?.toString() || ''
  );
  const [carbohydrates100g, setCarbs] = useState(
    initialNutriments?.carbohydrates100g?.toString() || ''
  );
  ...
  useEffect(() => {
    if (initialNutriments) {
      if (initialNutriments.energyKcal100g !== undefined)
        setEnergy(initialNutriments.energyKcal100g.toString());
      if (initialNutriments.fat100g !== undefined) setFat(initialNutriments.fat100g.toString());
      if (initialNutriments.saturatedFat100g !== undefined)
        setSatFat(initialNutriments.saturatedFat100g.toString());
      if (initialNutriments.carbohydrates100g !== undefined)
        setCarbs(initialNutriments.carbohydrates100g.toString());
      if (initialNutriments.sugars100g !== undefined)
        setSugars(initialNutriments.sugars100g.toString());
      if (initialNutriments.fiber100g !== undefined)
        setFiber(initialNutriments.fiber100g.toString());
      if (initialNutriments.proteins100g !== undefined)
        setProteins(initialNutriments.proteins100g.toString());
      if (initialNutriments.salt100g !== undefined) setSalt(initialNutriments.salt100g.toString());
    }
  }, [initialNutriments]);

  const handleConfirm = () => {
    Alert.alert('Save locally?', 'This will be saved to your device only.', [
